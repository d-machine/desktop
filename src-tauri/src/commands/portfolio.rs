...
pub fn get_portfolios() -> Result<Vec<Portfolio>, String> {
    let conn = db::acquire()?;
    let mut stmt = conn
        .prepare("SELECT portfolio_id, person_id, name, created_at FROM portfolios ORDER BY portfolio_id")
...
pub fn create_portfolio(input: CreatePortfolioInput) -> Result<Portfolio, String> {
    let conn = db::acquire()?;
    conn.execute(
        "INSERT INTO portfolios (name, person_id) VALUES (?1, ?2)",
        rusqlite::params![input.name, input.person_id],
    )
    .map_err(|e| e.to_string())?;
    ...
    let portfolio = conn
        .query_row(
            "SELECT portfolio_id, person_id, name, created_at FROM portfolios WHERE portfolio_id = ?1",
            [id],
            |row| {
                Ok(Portfolio {
                    portfolio_id: row.get(0)?,
